...
def get_all_answers(session_id: str) -> dict:
    """
    Retrieves all answers for a given session from Redis to create a complete lead.
    """
    if not redis_conn:
        return {}
    
    session_key = f"ctx:{session_id}"
    # hgetall returns a dictionary of bytes:bytes, so we decode them
    answers_bytes = redis_conn.hgetall(session_key)
